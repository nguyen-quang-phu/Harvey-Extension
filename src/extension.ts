...
export function activate(context: vscode.ExtensionContext) {
  ...
  let disposable = vscode.commands.registerCommand('extension.createStyleSheet', () => {
    ...
    const listStyles = fullText.match(/styles.([a-zA-Z]+)/g)?.map((nameClass) => {
    ...
    editor.edit((editBuilder) => {
      editBuilder.replace(
        new vscode.Range(
          ...
          new vscode.Position(lineContainStyleSheet + 1, 0),
        ),
